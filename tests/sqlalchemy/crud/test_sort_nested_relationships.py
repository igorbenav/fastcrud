...
async def test_sort_nested_one_to_many_relationships(async_session):
    ...
    assert [article["title"] for article in author1_data["articles"]] == ["A Article", "B Article", "C Article"]
    ...
    author1_data = next(item for item in result["data"] if item["id"] == 1)
    assert [article["title"] for article in author1_data["articles"]] == ["C Article", "B Article", "A Article"]
    ...
    author1_data = next(item for item in result["data"] if item["id"] == 1)
    assert [article["published_date"] for article in author1_data["articles"]] == ["2023-01-01", "2023-02-01",
                                                                                   "2023-03-01"]
    ...
    author1_data = next(item for item in result["data"] if item["id"] == 1)
    assert [article["published_date"] for article in author1_data["articles"]] == ["2023-03-01", "2023-02-01",
                                                                                   "2023-01-01"]
